...
import { Edit, Plus, Trash2, Users, Shield, Mail, MoreVertical, AlertCircle } from "lucide-react"
...
export function UsersContent({ permissions }: UsersContentProps) {
  ...
  const [filteredUsers, setFilteredUsers] = useState<UserWithRolesAndPermissions[]>(users)
  ...
  const [userToDelete, setUserToDelete] = useState<UserWithRolesAndPermissions | null>(null)
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Gérez les utilisateurs et leurs rôles dans le système</p>
        </div>
      ...
      {showNoRolesAlert && (
        <Alert variant="destructive" className="border-l-4 border-red-500">
          ...
          <AlertTitle className="text-red-700 dark:text-red-300">Action Requise</AlertTitle>
          <AlertDescription className="text-red-600 dark:text-red-200">
            Vous devez créer au moins un rôle avant de pouvoir créer un utilisateur. Veuillez configurer des rôles dans
            la section "Rôles" pour continuer.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {permissions.includes(PERMISSIONS.USERS_VIEW) && (
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Utilisateurs actifs dans le système</p>
            </CardContent>
          </Card>
        )}
        {permissions.includes(PERMISSIONS.ROLES_VIEW) && (
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{rolesWithPermissions.length}</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Activité utilisateur</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="p-0">
            {filteredUsers.length === 0 ? (
              <div className="text-center py-12">
                ...
                {permissions.includes(PERMISSIONS.USERS_CREATE) && (
                  <>
                    <p className="text-muted-foreground">Commencez par créer votre premier utilisateur</p>
                ...
                <TableBody>
                  {filteredUsers.map((user) => (
                    <TableRow key={user.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-col flex-wrap gap-1">{getUserRolesBadges(user)}</div>
                      </TableCell>
                      <TableCell>
                        {user.createdAt ? (
                          <div>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                ...
                                className="gap-2"
                                disabled={!permissions.includes(PERMISSIONS.USERS_UPDATE)}
                              >
                              ...
                              <DropdownMenuItem
                                ...
                                disabled={!permissions.includes(PERMISSIONS.USERS_DELETE)}
                              >
